...
describe("Testing tmdb backend functionality", () => {
    it("should get popular movies", async () => {
        const response = await fetch(`http://localhost:${process.env.PORT}/api/api/tmdb/popular`)
    ...
    it("should find movies with a search term", async () => {
        ...
        const response = await fetch(`http://localhost:${process.env.PORT}/api/api/tmdb/search/${searchWord}`)
        const data = await response.json()
        expect(response.status).to.equal(200)
        expect(data).to.be.an("object").that.is.not.empty
        expect(data["results"][0]).to.include.all.keys(["title", "id"])
    ...
    it("should get movie details with movie id", async () => {
        ...
        const response = await fetch(`http://localhost:${process.env.PORT}/api/api/tmdb/details/${movieId}`)
        const data = await response.json()
        expect(response.status).to.equal(200)
        expect(data).to.be.an("object").that.is.not.empty
    ...
    it("Should get movie collection details", async () => {
        ...
        const response = await fetch(`http://localhost:${process.env.PORT}/api/api/tmdb/collection/${collectionID}`);
        const data = await response.json()
